...
public:
	...
	void MenuConfiguracion(int32 _NumeroDeConexiones = 4, FString  _ModoJuego = FString(TEXT("TodosContraTodos")), FString _PathSalaEspera = FString(TEXT("/Game/ThirdPerson/Maps/MapaEspera")));
...
protected:
	...
	UFUNCTION()
	void CallbackCompletadoCrearSesion(bool fueOk);
	...
	UFUNCTION()
	void CallbackCompletadoDestruirSesion(bool fueOk);
	UFUNCTION()
	void CallbackCompletadoEmpezarSesion(bool fueOk);

private:
	// Con esta propiedad enlazamos el boton de la interfaz grafica con esta variable. Ambos deben tener el mismo nombre
	UPROPERTY(meta = (BindWidget))
	class UButton* BotonAnfitrion;

	UPROPERTY(meta = (BindWidget))
	UButton* BotonUnirse;

	UFUNCTION()
	void BotonAnfitrionPulsado();

	UFUNCTION()
	void BotonUnirsePulsado();

	void MenuDeshacerConfiguracion();

	class USubsistemaInstanciaJuego* SubsistemaInstanciaJuego;

	int32 NumeroDeConexiones{4};
	FString ModoJuego{TEXT("TodosContraTodos")};
